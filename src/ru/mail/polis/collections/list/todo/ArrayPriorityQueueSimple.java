...
public class ArrayPriorityQueueSimple<E extends Comparable<E>> implements IPriorityQueue<E> {
    ...
    private boolean compar(int i, int j) {
        return comparator == null ? data[i].compareTo(data[j]) >= 0 : comparator.compare(data[i], data[j]) >= 0;
    }

    public void siftDown(int i) {
        int minIndex = i;
        if (2 * i + 1 < size() && compar(minIndex, 2 * i + 1)) {
            minIndex = 2 * i + 1;
        }
        if (2 * i + 2 < size() && compar(minIndex, 2 * i + 2)) {
            minIndex = 2 * i + 2;
        }
        if (minIndex != i) {
    ...
    public void swap(E[] mass, int i, int j) {
        E temp = mass[i];
        mass[i] = mass[j];
        mass[j] = temp;
    }
